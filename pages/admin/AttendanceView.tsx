                                ...
                                {activeEmployees.map(emp => {
                                    ...
                                    return (
                                        <div 
                                            ...
                                            className="font-bold border-b border-r flex items-center justify-center text-center p-1 break-keep h-[70px] md:h-[50px]"
                                        >
                                            <span className="md:writing-mode-initial [writing-mode:vertical-rl] md:rotate-0 rotate-180">{emp.name}</span>
                                        </div>
                                    );
                                })}
                            </div>
                        ...
                        <div className="flex">
                            ...
                            <div className="flex-1 grid" style={{ gridTemplateColumns }}>
                                {activeEmployees.map(employee => (
                                    <div key={employee.id} className="relative border-r cursor-pointer" onClick={() => handleEmptyCellClick(employee.id)}>
                                        {Array.from({ length: 24 }).map((_, hour) => (
                                        ...
                                        {dayRecords.filter(rec => rec.employeeId === employee.id).map(rec => {
                                            ...
                                            if (rec.breakStart && rec.breakEnd) {
                                                ...
                                                if (breakStartDecimal >= startDecimal && breakEndDecimal <= endDecimal) {
                                                    const breakTop = (breakStartDecimal - startDecimal) * hourHeight;
                                                    const breakHeight = (breakEndDecimal - breakStartDecimal) * hourHeight;
                                                    breakBlock = (
                                                        <div 
                                                            className="absolute left-0 w-full bg-slate-500/50 flex items-center justify-center"
                                                            style={{ top: `${breakTop}px`, height: `${breakHeight}px` }}
                                                        >
                                                            <span className="text-white text-[10px] font-semibold">휴게</span>
                                                        </div>
                                                    );
                                                }
                                            }
